...
#---------------------------------------------------------------------------------
CFLAGS	:=  -std=c17   -Wall -Wno-array-bounds -Wno-stringop-overflow -Wno-stringop-overread -DPLATFORM_WIIU -fshort-wchar $(INCLUDE) 
CXXFLAGS := -std=c++20 -Wall -Wno-array-bounds -Wno-stringop-overflow -Wno-stringop-overread -DPLATFORM_WIIU -fshort-wchar $(INCLUDE)
ASFLAGS	:= -mregnames
LDFLAGS	:= -nostartfiles -Wl,-Map,$(notdir $@).map,--gc-sections,--wrap=malloc,--wrap=free,--wrap=calloc,--wrap=realloc,--wrap=aligned_alloc,--wrap=malloc_usable_size,--wrap=memalign,--wrap=posix_memalign,--wrap=pvalloc,--wrap=valloc,--wrap=printf,--wrap=puts
...
LIBDIRS	:=	$(CURDIR)	\
			$(DEVKITPPC)/lib  \
			$(DEVKITPPC)/lib/gcc/powerpc-eabi/11.1.0\
			$(PORTLIBS)
...
#---------------------------------------------------------------------------------
executable_d:
	@echo -e "\n${_YELLOW}Building for dev/debug. DO NOT RELEASE${_END}"
...
debug: CFLAGS += -D__DEBUG__ -O3
debug: CXXFLAGS += -D__DEBUG__ -O3
debug: $(OUTPUT).elf
...
%.elf: link.ld $(OFILES)
	...
	@cp $@ ../build_game_files_archive/content/turbo_ctgp
