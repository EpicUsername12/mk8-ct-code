...
void ctgp::crypto::CryptoEngine::AES_Encrypt16(uint8_t *key, uint8_t *iv,
                                               uint8_t *source, uint8_t *dest,
                                               size_t length) {
  symmetric_CBC cbc;
  cbc_start(find_cipher("aes"), static_cast<const uint8_t *>(iv),
            static_cast<const uint8_t *>(key), 16, 0, &cbc);
  cbc_encrypt(source, dest, length, &cbc);
  cbc_done(&cbc);
}

void ctgp::crypto::CryptoEngine::AES_Encrypt32(uint8_t *key, uint8_t *iv,
                                               uint8_t *source, uint8_t *dest,
                                               size_t length) {
  symmetric_CBC cbc;
  cbc_start(find_cipher("aes"), static_cast<const uint8_t *>(iv),
            static_cast<const uint8_t *>(key), 32, 0, &cbc);
  cbc_encrypt(source, dest, length, &cbc);
  cbc_done(&cbc);
}

void ctgp::crypto::CryptoEngine::AES_Decrypt16(uint8_t *key, uint8_t *iv,
                                               uint8_t *source, uint8_t *dest,
                                               size_t length) {
  symmetric_CBC cbc;
  cbc_start(find_cipher("aes"), static_cast<const uint8_t *>(iv),
...
void ctgp::crypto::CryptoEngine::AES_Decrypt32(uint8_t *key, uint8_t *iv,
                                               uint8_t *source, uint8_t *dest,
                                               size_t length) {
  symmetric_CBC cbc;
  cbc_start(find_cipher("aes"), static_cast<const uint8_t *>(iv),
...
void ctgp::crypto::CryptoEngine::RC4_EncryptDecrypt(uint8_t *key, size_t keylen,
                                                    uint8_t *source,
                                                    uint8_t *dest,
                                                    size_t data_len,
                                                    size_t offset) {
  internal_rc4_state st;
  internal_rc4_init(&st, key, keylen),
      internal_rc4_crypt(&st, source, dest, data_len, offset);
}
