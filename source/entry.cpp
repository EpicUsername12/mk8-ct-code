...
extern "C" int __entry(int argc, char** argv) {
    ...
    if (alreadyDoneIOSKernelExploit && currentTitleId != MK8_J && currentTitleId != MK8_U && currentTitleId != MK8_E) {
    ...
    if ((currentTitleId != MK8_J) && (currentTitleId != MK8_U) && (currentTitleId != MK8_E)) {
        C_UNLESS(!SYSCheckTitleExists(MK8_J), SYSLaunchTitle(MK8_J); found_one_title = true);
        C_UNLESS(!SYSCheckTitleExists(MK8_U), SYSLaunchTitle(MK8_U); found_one_title = true);
        C_UNLESS(!SYSCheckTitleExists(MK8_E), SYSLaunchTitle(MK8_E); found_one_title = true);
        C_UNLESS(found_one_title, SYSLaunchMenu());
        return RETURN_TO_NEXT_APP;
    }

    OSCreateThread(mainCtgpThread, (void*)MainThread, argc, (char*)argv, (void*)((uint32_t)stack + 0x8000), 0x2000, 0, OS_THREAD_ATTRIB_AFFINITY_CPU1 | OS_THREAD_ATTRIB_DETACHED);
