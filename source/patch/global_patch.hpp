...
typedef struct ControlButtonHandlerRedirect {
  ...
  ControlButtonHandlerRedirect(int pageId,
                               bool (*f)(ui::Control_Button *, ui::UIEvent *)) {
    m_pageId = pageId;
    m_RedirectFunc = f;
  }
...
public:
   ...
   * structure
   */
  void RegisterButtonHandlerRedirection(
      ControlButtonHandlerRedirect &buttonRedirection) {
   ...
   * @param f Function pointer
   */
  void RegisterButtonHandlerRedirection(int pageId,
                                        bool (*f)(ui::Control_Button *,
                                                  ui::UIEvent *)) {
    m_ButtonHandlerRedirections.push_back(
        ControlButtonHandlerRedirect(pageId, f));
  }
   ...
   * @param f Function pointer
   */
  void RegisterInputHandlerRedirection(bool (*f)(ui::UIInput *, int, int,
                                                 uint32_t)) {
   ...
   * @param val Boolean
   */
  void setOnlineToggle(bool val) { m_OnlineToggle = val; }
   ...
   * @return Boolean
   */
  bool getOnlineToggle() { return m_OnlineToggle; }
