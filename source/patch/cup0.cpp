...
ctgp::CUP0::CUP0() {
    ...
    memcpy(this->cupMessageIdSave, this->cupMessageId, sizeof(this->cupMessageIdSave));
}

void ctgp::CUP0::Initialize() {

    this->data = (uint8_t*)GetArchiveFile("/vol/content/data.cup0", NULL, 0x1000);
    ...
    char* stringTable = (char*)(this->data + sizeof(CUP0_Header) + (sizeof(CUP0_CupEntries) * header->numCupEntries) + (sizeof(CUP0_TrackEntries) * header->numTrackEntries));
    CUP0_CupEntries* cupEntries = (CUP0_CupEntries*)(this->data + sizeof(CUP0_Header));
    CUP0_TrackEntries* trackEntries = (CUP0_TrackEntries*)(this->data + sizeof(CUP0_Header) + (sizeof(CUP0_CupEntries) * header->numCupEntries));
    ...
    for (size_t i = 0; i < header->numCupEntries; i++) {
        ...
        this->pages[pageId]->cupBflimFile[cupId] = (nw::lyt::TextureInfo*)calloc(1, sizeof(nw::lyt::TextureInfo));
        if (!nw::lyt::LoadTexture(this->pages[pageId]->cupBflimFile[cupId], cupData, outSize)) {
    ...
    for (size_t i = 0; i < header->numTrackEntries; i++) {
        ...
        if (strlen(trackNameRetro) > 0) {
            ...
            this->pages[pageId]->retroTrackNames[cupId][trackEntries[i].trackId] = 15937 + i;
        }

        uint32_t outSize = 0;
        uint8_t* trackData = (uint8_t*)GetArchiveFile(trackPic, &outSize, 0x1000);
        this->pages[pageId]->trackBflimFiles[cupId][trackEntries[i].trackId] = (nw::lyt::TextureInfo*)calloc(1, sizeof(nw::lyt::TextureInfo));
        if (!nw::lyt::LoadTexture(this->pages[pageId]->trackBflimFiles[cupId][trackEntries[i].trackId], trackData, outSize)) {
