...
DECL_CXX_HOOK(int, net::NetworkData_Menu::getRouletteResult, void *_this) {
  ...
  if (ctgp::GlobalPatchInstance->getOnlineToggle() &&
      nn::pia::transport::StationManager::getInstance()
              ->localStation->stationIndex ==
          currentNetworkData->m_CourseChoice.m_PeerInfo.m_StationId) {
    for (int i = 0; i < 12; i++) {
      if (currentNetworkData->m_Players[i].m_PeerInfo.m_StationId != 253) {
        ...
        ctgp::OnlinePatch::SendToStation(
            currentNetworkData->m_Players[i].m_PeerInfo.m_StationId, buffer);
        free(buffer);
      }
    }
  }
...
void loadPageTrackPaths() {
  ...
  if (ctgp::CoursePatchInstance->m_CurrentPageNum > 0) {
    for (int i = 0; i < 8; i++) {
      for (int j = 0; j < 4; j++) {
        ctgp::CUP0Instance->trackNamesMod
            [heap->courseData->courseList.cups[i].courses[j].id] =
            ctgp::CUP0Instance
    ...
    for (int i = 0; i < 8; i++) {
      for (int i = 0; i < 4; i++) {
        ctgp::CUP0Instance->trackNamesMod
            [heap->courseData->courseList.cups[i].courses[i].id] = NULL;
      }
    }
  }
}

DECL_CXX_HOOK(int, nn::pia::inet::SocketInputStream::readFromSocket,
              nn::pia::inet::SocketInputStream *_this, uint32_t *data) {
  int ret = real(_this, data);
  if (!ret && ctgp::GlobalPatchInstance->getOnlineToggle() &&
      data[0] == 0x43544750) {
...
DECL_CXX_HOOK(int, nn::nex::MatchmakeSessionSearchCriteria::SetAttribute,
              nn::nex::MatchmakeSessionSearchCriteria *_this,
              uint32_t attributeIndex, uint32_t attributeValue) {
...
DECL_CXX_HOOK(int, nn::nex::MatchmakeSession::SetAttribute,
              nn::nex::MatchmakeSession *_this, uint32_t attributeIndex,
...
DECL_CXX_HOOK(int, ui::Page_WifiTop::onCreate, ui::Page_WifiTop *_this) {
  ...
  _this->createControl_(onlineCtgpToggleBtn, _this->controls.mPtrs[0],
                        "L_CTGP_00");
...
bool Page_WifiTop_ButtonHandler(ui::Control_Button *_this, ui::UIEvent *event) {
  ...
  if (onlineCtgpToggleBtn &&
      _this == onlineCtgpToggleBtn) { // 1 = A press, 0 = Touch pad
    if (event->m_InputValue <= 1 || event->m_InputValue == VPAD_BUTTON_PLUS) {
      ctgp::GlobalPatchInstance->setOnlineToggle(
...
DECL_CXX_HOOK(int, enl::PiaInetMatchingManager::autoMatch,
...
DECL_CXX_HOOK(int, enl::PiaInetMatchingManager::joinMatch,
              enl::PiaInetMatchingManager *_this) {
  int ret = real(_this);
  /*
          if(ctgp::GlobalPatchInstance->m_IsRgsConnected) {
...
DECL_CXX_HOOK(int, enl::PiaInetMatchingManager::createMatch,
              enl::PiaInetMatchingManager *_this) {
  int ret = real(_this);
  /*
          if(ctgp::GlobalPatchInstance->m_IsRgsConnected) {
...
DECL_CXX_HOOK(int, enl::PiaInetMatchingManager::disconnect,
              enl::PiaInetMatchingManager *_this) {
  int ret = real(_this);
  /*
          if(ctgp::GlobalPatchInstance->m_IsRgsConnected) {
...
DECL_CXX_HOOK(int, ui::UICourseList::getRandomOnlineTracks,
              ui::UICourseList *_this, ui::CourseInfo *trackOutputs,
              void *blacklist) {
  memset(ctgp::OnlinePatchInstance->m_TrackOptionsPage, 0,
         sizeof(ctgp::OnlinePatchInstance->m_TrackOptionsPage));
...
void ctgp::OnlinePatch::Initialize() {
  ...
  ctgp::GlobalPatchInstance->RegisterButtonHandlerRedirection(
      ui::Page_WifiTop::ID, Page_WifiTop_ButtonHandler);
  ...
  MK8_HOOK_CXX_FUNC(0x02A02944,
                    nn::pia::inet::SocketInputStream::readFromSocket);
  MK8_HOOK_CXX_FUNC(0x0298E52C, nn::nex::MatchmakeSession::SetAttribute);
  MK8_HOOK_CXX_FUNC(0x0298E7B4,
                    nn::nex::MatchmakeSessionSearchCriteria::SetAttribute);
