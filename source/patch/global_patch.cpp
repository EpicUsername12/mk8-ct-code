...
DECL_CXX_HOOK(int, ui::UIPageManager::attach, ui::UIPageManager *_this,
              ui::UIPage *page, bool unk) {
...
DECL_CXX_HOOK(int, ui::Control_Button::onHandler, ui::Control_Button *_this,
              ui::UIEvent *event) {

  bool cancel = false;
  for (auto &ent : ctgp::GlobalPatchInstance->m_ButtonHandlerRedirections) {
    if ((ent.m_pageId == GLOBAL_PAGE_ID) ||
        (_this->ownerPage->pageID == ent.m_pageId)) {
...
DECL_CXX_HOOK(int, ui::UIInput::runHandler, ui::UIInput *_this, int issuer,
              int controllerIdx, uint32_t input) {
...
ctgp::GlobalPatch::GlobalPatch() { this->setName("ctgp::GlobalPatch"); }
