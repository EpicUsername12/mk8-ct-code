...
DECL_CXX_HOOK(int, ui::Page_Course::LoadPanes, ui::Page_Course *page,
              ui::UIControl *mainControl, char r5) {
  ...
  if (!ctgp::CoursePatchInstance->m_ButtonHolders.contains(page->pageID)) {
    ...
    ctgp::CoursePatchInstance->m_ButtonHolders.insert(
        std::make_pair(page->pageID, buttonHolder));
  }

  ui::Control_CupButton *controlL1 =
      ctgp::CoursePatchInstance->m_ButtonHolders[page->pageID].controlL1;
  ui::Control_CupButton *controlL2 =
      ctgp::CoursePatchInstance->m_ButtonHolders[page->pageID].controlL2;

  ui::Control_CupButton *controlR1 =
      ctgp::CoursePatchInstance->m_ButtonHolders[page->pageID].controlR1;
  ui::Control_CupButton *controlR2 =
      ctgp::CoursePatchInstance->m_ButtonHolders[page->pageID].controlR2;
  ...
  leftTexture = GetArchiveFile("/vol/content/custom/ym_CupIconLeft^h.bflim",
                               &outSize, 0x1000);
  leftTextureInfo =
      (nw::lyt::TextureInfo *)calloc(1, sizeof(nw::lyt::TextureInfo));
  nw::lyt::LoadTexture(leftTextureInfo, leftTexture, outSize);

  rightTexture = GetArchiveFile("/vol/content/custom/ym_CupIconRight^h.bflim",
                                &outSize, 0x1000);
  rightTextureInfo =
      (nw::lyt::TextureInfo *)calloc(1, sizeof(nw::lyt::TextureInfo));
  nw::lyt::LoadTexture(rightTextureInfo, rightTexture, outSize);

  controlL1->setTexture(controlL1->findPane("P_CupIcon_00"), leftTextureInfo,
                        1);
  controlL2->setTexture(controlL2->findPane("P_CupIcon_00"), leftTextureInfo,
                        1);

  controlR1->setTexture(controlR1->findPane("P_CupIcon_00"), rightTextureInfo,
                        1);
  controlR2->setTexture(controlR2->findPane("P_CupIcon_00"), rightTextureInfo,
                        1);

  memcpy(ctgp::CUP0Instance->trackNamesMod, ctgp::CUP0Instance->trackNames,
         sizeof(ctgp::CUP0Instance->trackNamesMod));
  memcpy(ctgp::CUP0Instance->cupMessageIdSave, ctgp::CUP0Instance->cupMessageId,
         sizeof(ctgp::CUP0Instance->cupMessageIdSave));

  return 1;
}

void *cupIconsBflimData[12] = {0};
const char *cupNamesInternal[] = {"Mushroom", "Flower", "Star",  "Special",
                                  "Shell",    "Banana", "Leaf",  "Thunder",
                                  "DLC02",    "DLC03",  "DLC04", "DLC05"};
void loadPage() {
  if (ctgp::CoursePatchInstance->m_CurrentPageNum == 0) {
    memcpy(ctgp::CUP0Instance->cupMessageId,
           ctgp::CUP0Instance->cupMessageIdSave,
           sizeof(ctgp::CUP0Instance->cupMessageIdSave));
    for (int i = 0; i < 8; i++) {
      ctgp::CUP0Instance->cupBflimFile[i]->texture.surface.image_data =
          ctgp::CUP0Instance->cupBflimFileSave[i].texture.surface.image_data;
      ctgp::CUP0Instance->cupBflimFile[i]->texture.surface.swizzle =
          ctgp::CUP0Instance->cupBflimFileSave[i].texture.surface.swizzle;
    }
  } else {
    ...
    for (int i = 0; i < 8; i++) {
      if (ctgp::CUP0Instance->pages[pageId]->pageCustomCups[i]) {
        ctgp::CUP0Instance->cupBflimFile[i]->texture.surface.image_data =
            ctgp::CUP0Instance->pages[pageId]
                ->cupBflimFile[i]
                ->texture.surface.image_data;
        ctgp::CUP0Instance->cupBflimFile[i]->texture.surface.swizzle =
            ctgp::CUP0Instance->pages[pageId]
                ->cupBflimFile[i]
...
void increment_page() {
  ...
  if (ctgp::CoursePatchInstance->m_CurrentPageNum ==
      (int)ctgp::CUP0Instance->numPages)
...
void ctgp::CoursePatch::LoadPage() { loadPage(); }
...
DECL_CXX_HOOK(int, ui::UICursorLink::next, ui::UICursorLink *link,
              unsigned int input, int currentPos) {
...
std::vector<std::tuple<const char *, nw::lyt::TextureInfo *>>
    customBattleTracks;
...
void handle_battle_swap() {
  ...
  if (ctgp::CoursePatchInstance->m_CustomBattle) {
    for (int i = 0; i < 8; i++) {
      DBG_LOG(
          "%d: %d %s\n", i, page->m_CourseButtons[i]->courseId,
          ctgp::CUP0Instance->trackNames[page->m_CourseButtons[i]->courseId]);
      const char *path;
  ...
  if (!ctgp::CoursePatchInstance->m_CustomBattle) {
    for (int i = 0; i < 8; i++) {
      *page->m_CourseButtons[i] = backupPageBattle[i];
      ctgp::CUP0Instance->trackNamesMod[page->m_CourseButtons[i]->courseId] =
          nullptr;

      ui::Heap_Common *heap = ui::Heap_Common::getHeap();
      for (int cupIdx = 0; cupIdx < 12; cupIdx++) {
        for (int trackIdx = 0; trackIdx < 8; trackIdx++) {
          if (heap->courseData->courseList.cups[cupIdx].courses[trackIdx].id ==
              page->m_CourseButtons[i]->courseId) {
            page->m_CourseButtons[i]->setCourse(
                &heap->courseData->courseList.cups[cupIdx].courses[trackIdx],
                nullptr);
            break;
          }
        }
...
DECL_CXX_HOOK(int, ui::CursorLink_CourseMatrix::next,
              ui::CursorLink_CourseMatrix *_this, uint32_t input,
              int currentPos) {
...
void changeCourseButtons(ui::Page_Course *page, int cupId) {
  ...
  ui::Heap_Common *heap = ui::Heap_Common::getHeap();
  if (ctgp::CoursePatchInstance->m_CurrentPageNum > 0 && cupId < 8 &&
      ctgp::CUP0Instance->pages[ctgp::CoursePatchInstance->m_CurrentPageNum - 1]
          ->pageCustomCups[cupId]) {
    for (int i = 0; i < 4; i++) {
      page->courseBtn[i]->setTexture(
          page->courseBtn[i]->coursePictPane,
          ctgp::CUP0Instance
              ->pages[ctgp::CoursePatchInstance->m_CurrentPageNum - 1]
              ->trackBflimFiles[cupId][i],
          1);
      ...
      page->courseBtn[i]->setMessage(
          page->courseBtn[i]->courseNameTextPane,
          ctgp::CUP0Instance
              ->pages[ctgp::CoursePatchInstance->m_CurrentPageNum - 1]
              ->trackNames[cupId][i]);
      if (ctgp::CUP0Instance
              ->pages[ctgp::CoursePatchInstance->m_CurrentPageNum - 1]
              ->retroTrackNames[cupId][i] > 0) {
        page->courseBtn[i]->setVisible(page->courseBtn[i]->classicNameTextPane,
                                       1);
        page->courseBtn[i]->setMessage(
            page->courseBtn[i]->classicNameTextPane,
            ctgp::CUP0Instance
                ->pages[ctgp::CoursePatchInstance->m_CurrentPageNum - 1]
                ->retroTrackNames[cupId][i]);
      } else {
        page->courseBtn[i]->setVisible(page->courseBtn[i]->classicNameTextPane,
                                       0);
      }
      page->mainControl->setMessage(
          page->cupNameTextPane,
          ctgp::CUP0Instance
              ->pages[ctgp::CoursePatchInstance->m_CurrentPageNum - 1]
              ->cupName[cupId]);
      ctgp::CUP0Instance->trackNamesMod
          [heap->courseData->courseList.cups[cupId].courses[i].id] =
          ctgp::CUP0Instance
              ->pages[ctgp::CoursePatchInstance->m_CurrentPageNum - 1]
              ->trackPaths[cupId][i];
    }
  ...
  if (ctgp::CoursePatchInstance->m_CurrentPageNum == 0) {
    for (int i = 0; i < 4; i++) {
      ctgp::CUP0Instance->trackNamesMod
          [heap->courseData->courseList.cups[cupId].courses[i].id] = nullptr;
    }
  }
}

DECL_CXX_HOOK(int, ui::Page_CourseBattle::onHandler,
              ui::Page_CourseBattle *_this, ui::UIEvent *event) {
...
DECL_CXX_HOOK(int, ui::Page_CourseGP::onHandler, ui::Page_CourseGP *_this,
              ui::UIEvent *event) {
  ...
  changeCourseButtons(_this,
                      ui::Heap_Common::getHeap()
                          ->courseData->courseRelatedData.currentlySelectedCup);
  return ret;
}

DECL_CXX_HOOK(int, ui::Page_Course44::onHandler, ui::Page_Course *_this,
              ui::UIEvent *event) {
  int ret = real(_this, event);
  changeCourseButtons(_this,
                      ui::Heap_Common::getHeap()
...
DECL_CXX_HOOK(int, ui::Page_Course63::onHandler, ui::Page_Course *_this,
              ui::UIEvent *event) {
  int ret = real(_this, event);
  changeCourseButtons(_this,
                      ui::Heap_Common::getHeap()
...
DECL_CXX_HOOK(void *, nw::lyt::MultiArcResourceAccessor::GetResource,
              nw::lyt::MultiArcResourceAccessor *_this, uint32_t resId,
              const char *path, uint32_t *out_size) {
...
DECL_CXX_HOOK(int, nw::lyt::LoadTexture, nw::lyt::TextureInfo *textureInfo,
              const void *filePtr, uint32_t size) {
  int ret = real(textureInfo, filePtr, size);
  for (int i = 0; i < 12; i++) {
    if (cupIconsBflimData[i] == filePtr) {
      ctgp::CUP0Instance->cupBflimFile[i] = textureInfo;
      memcpy(&ctgp::CUP0Instance->cupBflimFileSave[i], textureInfo,
             sizeof(nw::lyt::TextureInfo));
      break;
    }
  }
  return ret;
...
bool Global_ButtonHandler(ui::Control_Button *_this, ui::UIEvent *event) {
  for (auto &[pageId, ent] : ctgp::CoursePatchInstance->m_ButtonHolders) {
    if (pageId == _this->ownerPage->pageID) {
      if (_this == ent.controlL1 || _this == ent.controlL2 ||
          _this == ent.controlR1 || _this == ent.controlR2) {
...
DECL_CXX_HOOK(int, ui::UICursor::next_, ui::UICursor *_this, int r4, int r5,
              unsigned int input) {
  ...
  for (int j = 0; j < pageMgr->layouts.mSize; j++) {
    if (pageMgr->layouts.mBuffer[j] &&
        pageMgr->layouts.mBuffer[j]->cursor == _this &&
        (pageMgr->layouts.mBuffer[j]->pageID == 44 ||
         (pageMgr->layouts.mBuffer[j]->pageID == 63 &&
          ctgp::GlobalPatchInstance->getOnlineToggle()))) {
...
DECL_CXX_HOOK(void, ui::Page_CourseBattle::onCreate,
              ui::Page_CourseBattle *_this) {
  ...
  if (!ctgp::CoursePatchInstance->m_ButtonHolders.contains(_this->pageID)) {
    ...
    buttonHolder.controlR2 = new ui::Control_CupButton(1);
    ctgp::CoursePatchInstance->m_ButtonHolders.insert(
        std::make_pair(_this->pageID, buttonHolder));
  }

  ui::Control_CupButton *controlL1 =
      ctgp::CoursePatchInstance->m_ButtonHolders[_this->pageID].controlL1;
  ui::Control_CupButton *controlL2 =
      ctgp::CoursePatchInstance->m_ButtonHolders[_this->pageID].controlL2;

  ui::Control_CupButton *controlR1 =
      ctgp::CoursePatchInstance->m_ButtonHolders[_this->pageID].controlR1;
  ui::Control_CupButton *controlR2 =
      ctgp::CoursePatchInstance->m_ButtonHolders[_this->pageID].controlR2;
  ...
  void (*ui_UIControlSlider_push)(int _this, ui::UIControl *control) =
      (void (*)(int, ui::UIControl *))(0x025370A4 + MK8_TEXT_OFFSET);
...
void ctgp::CoursePatch::Initialize() {
  ...
  trackData = GetArchiveFile("/vol/content/custom/cPict_GDx_BtStadium.bflim",
                             &outSize, 0x1000);
  ...
  trackData = GetArchiveFile(
      "/vol/content/custom/cPict_GDx_BtSweetKingdom.bflim", &outSize, 0x1000);
  textureInfo = (nw::lyt::TextureInfo *)calloc(1, sizeof(nw::lyt::TextureInfo));
  nw::lyt::LoadTexture(textureInfo, trackData, outSize);
  ...
  /* Dragon Palace entry */
  trackData = GetArchiveFile(
      "/vol/content/custom/cPict_GDx_BtDragonPalace.bflim", &outSize, 0x1000);
  textureInfo = (nw::lyt::TextureInfo *)calloc(1, sizeof(nw::lyt::TextureInfo));
  nw::lyt::LoadTexture(textureInfo, trackData, outSize);
  ...
  /* Lunar Colony entry */
  trackData = GetArchiveFile(
      "/vol/content/custom/cPict_GDx_BtLunarColony.bflim", &outSize, 0x1000);
  textureInfo = (nw::lyt::TextureInfo *)calloc(1, sizeof(nw::lyt::TextureInfo));
  nw::lyt::LoadTexture(textureInfo, trackData, outSize);
  customBattleTracks.push_back(std::make_tuple("Gu_BtMoon", textureInfo));

  /* Wuhu Town entry */
  trackData = GetArchiveFile("/vol/content/custom/cPict_GDx_BtWuhuTown.bflim",
                             &outSize, 0x1000);
  textureInfo = (nw::lyt::TextureInfo *)calloc(1, sizeof(nw::lyt::TextureInfo));
  nw::lyt::LoadTexture(textureInfo, trackData, outSize);
  ...
  trackData = GetArchiveFile(
      "/vol/content/custom/cPict_GDx_BtLuigiMansion.bflim", &outSize, 0x1000);
  textureInfo = (nw::lyt::TextureInfo *)calloc(1, sizeof(nw::lyt::TextureInfo));
  nw::lyt::LoadTexture(textureInfo, trackData, outSize);
  customBattleTracks.push_back(
      std::make_tuple("Gu_BtLuigiMansion", textureInfo));

  /* Battle Course 1 entry */
  trackData = GetArchiveFile(
      "/vol/content/custom/cPict_GDx_BtBattleCourse1.bflim", &outSize, 0x1000);
  textureInfo = (nw::lyt::TextureInfo *)calloc(1, sizeof(nw::lyt::TextureInfo));
  nw::lyt::LoadTexture(textureInfo, trackData, outSize);
  customBattleTracks.push_back(std::make_tuple("Gu_BtBattle1", textureInfo));

  /* Urchin Underpass entry */
  trackData = GetArchiveFile("/vol/content/custom/cPict_GDx_BtUrchin.bflim",
                             &outSize, 0x1000);
  textureInfo = (nw::lyt::TextureInfo *)calloc(1, sizeof(nw::lyt::TextureInfo));
  nw::lyt::LoadTexture(textureInfo, trackData, outSize);
  ...
  ctgp::GlobalPatchInstance->RegisterButtonHandlerRedirection(
      GLOBAL_PAGE_ID, Global_ButtonHandler);
