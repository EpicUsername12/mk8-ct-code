...
void pygecko_memcpy(unsigned char *dst, unsigned char *src, unsigned int len) {
  ...
  SC0x25_KernelCopyData((unsigned int)OSEffectiveToPhysical(dst),
                        (unsigned int)&memcpy_buffer, len);
...
uint32_t doBLA(uint32_t addr) { return (0x48000003 | (addr & 0x03FFFFFC)); }

PatchedFunction *hookBranch(uint32_t source, void *destination,
                            uint32_t delta) {
  ...
  memcpy(&pFuncList[ret_idx]->sourceInstructions, (void *)(source + delta),
         10 * sizeof(uint32_t));

  if ((uint32_t)destination <= 0x03FFFFFC) {
    Write32((uint32_t *)(source + delta), doBLA((uint32_t)destination));
  } else {
    Write32((uint32_t *)(source + delta),
            doBL((uint32_t)destination, (source + delta)));
  }
...
PatchedFunction *hookBranchNoLink(uint32_t source, void *destination,
  ...
  if ((uint32_t)destination <= 0x03FFFFFC) {
    ...
    Write32((uint32_t *)(source + delta),
            doBL((uint32_t)destination, (source + delta)) ^ 1);
  }
...
PatchedFunction *hookFunction(void *source, void *destination, uint32_t delta,
  ...
  pFuncList[ret_idx]->destAddr = (uint32_t)destination;
  memcpy(&pFuncList[ret_idx]->sourceInstructions,
         (void *)((uint32_t)source + delta), 10 * sizeof(uint32_t));

  delta += 4;
  hookJumps[ret_idx][0] = PowerPC_STW(gpr3, gpr1, -32); // stw r3, -32(r1)
  hookJumps[ret_idx][1] =
      PowerPC_LIS(gpr3, ((int)source + delta) >> 16); // lis r3, source@h
  hookJumps[ret_idx][2] =
      PowerPC_ORI(gpr3, gpr3, ((int)source + delta));   // ori r3, r3, source@l
  hookJumps[ret_idx][3] = PowerPC_MTSPR(gpr3, SPR_CTR); // mtctr r3
  hookJumps[ret_idx][4] = PowerPC_LWZ(gpr3, gpr1, -32); // lwz r3, -32(r1)
  hookJumps[ret_idx][5] = PowerPC_MFSPR(
      gpr13, SPR_LR); // mflr r13 (so you can read LR from the hook function)
  hookJumps[ret_idx][6] =
      pFuncList[ret_idx]
          ->sourceInstructions[0]; // base func first instruction (because the
...
void removePatch(PatchedFunction *ptr) {
  ...
  R_UNLESS(ret_idx != 64, );
  MemoryCopy((uint32_t *)pFuncList[ret_idx]->sourceAddr,
             &pFuncList[ret_idx]->sourceInstructions, 10 * sizeof(uint32_t));
