...
int (*OSDynLoad_Acquire)(const char *dynLoadExeName,
                         uint32_t *outHandle) = (int (*)(const char *,
                                                         uint32_t *))0x0102A3B4;
int (*OSDynLoad_FindExport)(uint32_t handle, bool isFromDataSection,
                            const char *name, void **fpptr) =
    (int (*)(uint32_t, bool, const char *, void **))0x0102B828;
...
void *(*memoryAllocAlign)(size_t size,
                          size_t align) = (void *(*)(size_t, size_t))0x1048d10;
...
int (*FSOpenFile)(FSClient *client, FSCmdBlock *block, const char *path,
                  const char *mode, int *handle, uint32_t flags);
int (*FSCloseFile)(FSClient *client, FSCmdBlock *block, int handle,
                   uint32_t unk1);
int (*FSOpenDir)(FSClient *client, FSCmdBlock *block, const char *dirPath,
                 int *dirHandle, uint32_t flags);
int (*FSReadDir)(FSClient *client, FSCmdBlock *block, int dirHandle,
                 FSDirEntry *returnedDirEntry, int errHandling);
int (*FSCloseDir)(FSClient *client, FSCmdBlock *block, int dirHandle, int unk1);
int (*FSGetStat)(FSClient *client, FSCmdBlock *block, const char *path,
                 FSStat *stat, uint32_t flags);
int (*FSGetStatFile)(FSClient *client, FSCmdBlock *block, int handle,
                     FSStat *stat, uint32_t flags);
int (*FSReadFile)(FSClient *client, FSCmdBlock *block, void *buffer,
                  uint32_t count, size_t size, int handle, uint32_t unk1,
                  uint32_t flags);
int (*FSReadFileWithPos)(FSClient *client, FSCmdBlock *block, uint8_t *buffer,
                         uint32_t count, size_t size, uint32_t pos, int handle,
                         uint32_t unk1, uint32_t flags);
int (*FSSetPosFile)(FSClient *client, FSCmdBlock *block, int handle,
                    uint32_t pos, uint32_t flags);
int (*FSGetMountSource)(FSClient *client, FSCmdBlock *cmd,
                        int type /* 0 = sd */, char *out, uint32_t flags);
int (*FSMount)(FSClient *client, FSCmdBlock *cmd, char *source,
               const char *target, uint32_t bytes, uint32_t flags);
int (*FSGetPosFile)(FSClient *client, FSCmdBlock *block, int fileHandle,
                    uint32_t *pos, uint32_t flags);

int (*IOS_Open)(const char *devNode, int mode);
int (*IOS_Close)(int handle);
int (*IOS_Ioctl)(int handle, uint32_t methodId, void *bufIn, size_t sizeIn,
                 void *bufIo, size_t sizeIo);

bool (*OSCreateThread)(OSThread *thread, void *entry, int argc, char *argv,
                       void *stack, uint32_t stackSize, int priority,
                       uint16_t attributes);
...
int (*sendto)(int s, const void *buffer, int size, int flags,
              const sockaddr_in *dest, int dest_len);
int (*setsockopt)(int s, int level, int optname, void *optval, int optlen);
int (*bind)(int sockfd, struct sockaddr *my_addr, int addrlen);
int (*listen)(int s, int backlog);
hostent *(*gethostbyname)(const char *ipAddr);
int (*accept)(int sock, struct sockaddr *addr, size_t *len);
int (*send)(int s, const void *msg, size_t len, int flags);
int (*socketlasterr)(void);
int (*recv)(int s, void *buf, int len, unsigned int flags);
const char *(*inet_ntoa)(in_addr addr);

void (*OSInitMutexEx)(OSMutex *mutex, const char *name);
bool (*OSLockMutex)(OSMutex *mutex);
bool (*OSUnlockMutex)(OSMutex *mutex);
uint64_t (*OSGetSystemTime)(void);
...
int (*OSGetUPID)(void);
bool (*OSSetExceptionCallbackEx)(OSExceptionMode mode,
                                 OSExceptionType exceptionType, void *callback);
...
int (*SAVEOpenFile)(FSClient *client, FSCmdBlock *block, uint8_t accountSlotNo,
                    const char *path, const char *mode, int *fileHandle,
                    int errHandling);
...
uint8_t (*nn::act::GetSlotNo)(void);
int (*nn::act::GetAccountIdEx)(char *pOutAccountId, uint8_t slotNum);
int (*nn::act::GetMiiName)(char16_t *pOutMiiName);

/*
 * https://www.gnu.org/software/libc/manual/html_node/Replacing-malloc.html
 */

extern "C" void *__wrap_malloc(size_t size) { return memoryAlloc(size); }

extern "C" void __wrap_free(void *ptr) { memoryFree(ptr); }
...
extern "C" size_t __wrap_malloc_usable_size(void *p) { return 0x7FFFFFFF; }

extern "C" void *__wrap_memalign(size_t alignment, size_t size) {
  return __wrap_aligned_alloc(alignment, size);
}

extern "C" int __wrap_posix_memalign(void **memptr, size_t alignment,
                                     size_t size) {
...
void LoadWiiUSymbols() {
  ...
  uint32_t __rpl_coreinit, __rpl_nsysnet, __rpl_sysapp, __rpl_vpad,
      __rpl_nn_save, __rpl_proc_ui, __rpl_gx2, __rpl_nn_act;
  ...
  OSDynLoad_Acquire("nn_act.rpl", &__rpl_nn_act);
  ...
  LOAD_FUNC(__rpl_nsysnet, bind);
  LOAD_FUNC(__rpl_nsysnet, listen);
  LOAD_FUNC(__rpl_nsysnet, gethostbyname);
  LOAD_FUNC(__rpl_nsysnet, accept);
  LOAD_FUNC(__rpl_nsysnet, send);
  LOAD_FUNC(__rpl_nsysnet, socketlasterr);
  LOAD_FUNC(__rpl_nsysnet, recv);
  LOAD_FUNC(__rpl_nsysnet, inet_ntoa);

  LOAD_FUNC(__rpl_coreinit, OSInitMutexEx);
  LOAD_FUNC(__rpl_coreinit, OSLockMutex);
  LOAD_FUNC(__rpl_coreinit, OSUnlockMutex);
  LOAD_FUNC(__rpl_coreinit, OSGetSystemTime);
  ...
  __load_from_module(__rpl_nn_act, "GetSlotNo__Q2_2nn3actFv",
                     (void **)&nn::act::GetSlotNo);
  __load_from_module(__rpl_nn_act, "GetAccountIdEx__Q2_2nn3actFPcUc",
                     (void **)&nn::act::GetAccountIdEx);
  __load_from_module(__rpl_nn_act, "GetMiiName__Q2_2nn3actFPw",
                     (void **)&nn::act::GetMiiName);
