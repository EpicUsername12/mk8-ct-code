...
void ExceptionHandler(const char *type, OSContext *context) {
  ...
  __os_snprintf(buffer, 2048,
                ...
                type, symbolname, OSGetTitleID(), OSGetCoreId(), OSGetUPID(), 0,
                context->gpr[0], 11, context->gpr[11], 22, context->gpr[22], 1,
                context->gpr[1], 12, context->gpr[12], 23, context->gpr[23], 2,
                context->gpr[2], 13, context->gpr[13], 24, context->gpr[24], 3,
                context->gpr[3], 14, context->gpr[14], 25, context->gpr[25], 4,
                context->gpr[4], 15, context->gpr[15], 26, context->gpr[26], 5,
                context->gpr[5], 16, context->gpr[16], 27, context->gpr[27], 6,
                context->gpr[6], 17, context->gpr[17], 28, context->gpr[28], 7,
                context->gpr[7], 18, context->gpr[18], 29, context->gpr[29], 8,
                context->gpr[8], 19, context->gpr[19], 30, context->gpr[30], 9,
                context->gpr[9], 20, context->gpr[20], 31, context->gpr[31], 10,
                context->gpr[10], 21, context->gpr[21], lrBuffer, context->srr0,
                context->dar, context->dsisr);
...
void ctgp::common::UDPDebugger::Initialize() {
  ...
  setsockopt(this->m_SocketFd, SOL_SOCKET, SO_BROADCAST, &broadcastEnable,
  ...
  printf("\n\nStarted debug logger.\nProject: %s v%d.%d.%d-%s by %s\n\n",
         PROJECT_NAME, PROJECT_VER_MAJOR, PROJECT_VER_VERSION,
         PROJECT_VER_MINOR, PROJECT_VER_DISC, PROJECT_AUTHOR);
  OSSetExceptionCallbackEx(OS_EXCEPTION_MODE_GLOBAL_ALL_CORES,
                           OS_EXCEPTION_TYPE_DSI, (void *)ExcDSI);
  OSSetExceptionCallbackEx(OS_EXCEPTION_MODE_GLOBAL_ALL_CORES,
                           OS_EXCEPTION_TYPE_ISI, (void *)ExcISI);
  OSSetExceptionCallbackEx(OS_EXCEPTION_MODE_GLOBAL_ALL_CORES,
                           OS_EXCEPTION_TYPE_PROGRAM, (void *)ExcProgram);
#endif
}
...
void ctgp::common::UDPDebugger::PrintLog(const char *txt) {
  ...
  while (len > 0) {
    ...
    ret = sendto(this->m_SocketFd, txt, block, 0, &this->m_SocketAddr,
                 sizeof(sockaddr_in));
    if (ret < 0)
      break;
...
int (*__os_vsnprintf)(char *buffer, size_t size, const char *fmt,
                      va_list) = (int (*)(char *, size_t, const char *,
                                          va_list))(0x2012D18 - 0xFE3C00);
