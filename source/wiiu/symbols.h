...
#define __os_snprintf                                                          \
  ((int (*)(char *s, int n, const char *format, ...))0x0102F160)
...
typedef struct {
  ...
  uint16_t invalid; /* 0 = All valid, 1 = X invalid, 2 = Y invalid, 3 = Both
                       invalid? */
} VPADTPData;
...
#define OSMillisecondsToTicks(val)                                             \
  (((uint64_t)(val) * (uint64_t)OSTimerClockSpeed) / 1000ull)
#define OSMicrosecondsToTicks(val)                                             \
  (((uint64_t)(val) * (uint64_t)OSTimerClockSpeed) / 1000000ull)
#define OSNanosecondsToTicks(val)                                              \
  (((uint64_t)(val) * ((uint64_t)OSTimerClockSpeed) / 31250ull) / 32000ull)

#define OSTicksToSeconds(val) ((uint64_t)(val) / (uint64_t)OSTimerClockSpeed)
#define OSTicksToMilliseconds(val)                                             \
  (((uint64_t)(val)*1000ull) / (uint64_t)OSTimerClockSpeed)
#define OSTicksToMicroseconds(val)                                             \
  (((uint64_t)(val)*1000000ull) / (uint64_t)OSTimerClockSpeed)
#define OSTicksToNanoseconds(val)                                              \
  (((uint64_t)(val)*32000ull) / ((uint64_t)OSTimerClockSpeed / 31250ull))
...
#define OS_THREAD_ATTRIB_AFFINITY_ANY                                          \
  OS_THREAD_ATTRIB_AFFINITY_CPU0 | OS_THREAD_ATTRIB_AFFINITY_CPU1 |            \
...
typedef struct OSDynLoad_RPLInfo {
...
extern void *(*memoryAllocAlign)(size_t size, size_t align);
...
extern int (*FSOpenFile)(FSClient *client, FSCmdBlock *block, const char *path,
                         const char *mode, int *handle, uint32_t flags);
extern int (*FSCloseFile)(FSClient *client, FSCmdBlock *block, int handle,
                          uint32_t unk1);
extern int (*FSOpenDir)(FSClient *client, FSCmdBlock *block,
                        const char *dirPath, int *dirHandle, uint32_t flags);
extern int (*FSReadDir)(FSClient *client, FSCmdBlock *block, int dirHandle,
                        FSDirEntry *returnedDirEntry, int errHandling);
extern int (*FSCloseDir)(FSClient *client, FSCmdBlock *block, int dirHandle,
                         int unk1);
extern int (*FSGetStat)(FSClient *client, FSCmdBlock *block, const char *path,
                        FSStat *stat, uint32_t flags);
extern int (*FSGetStatFile)(FSClient *client, FSCmdBlock *block, int handle,
                            FSStat *stat, uint32_t flags);
extern int (*FSReadFile)(FSClient *client, FSCmdBlock *block, void *buffer,
                         uint32_t count, size_t size, int handle, uint32_t unk1,
                         uint32_t flags);
extern int (*FSReadFileWithPos)(FSClient *client, FSCmdBlock *block,
                                uint8_t *buffer, uint32_t count, size_t size,
                                uint32_t pos, int handle, uint32_t unk1,
                                uint32_t flags);
extern int (*FSSetPosFile)(FSClient *client, FSCmdBlock *block, int handle,
                           uint32_t pos, uint32_t flags);
extern int (*FSGetMountSource)(FSClient *client, FSCmdBlock *cmd,
                               int type /* 0 = sd */, char *out,
                               uint32_t flags);
extern int (*FSMount)(FSClient *client, FSCmdBlock *cmd, char *source,
                      const char *target, uint32_t bytes, uint32_t flags);
extern int (*FSGetPosFile)(FSClient *client, FSCmdBlock *block, int fileHandle,
                           uint32_t *pos, uint32_t flags);

extern int (*IOS_Open)(const char *devNode, int mode);
extern int (*IOS_Close)(int handle);
extern int (*IOS_Ioctl)(int handle, uint32_t methodId, void *bufIn,
                        size_t sizeIn, void *bufIo, size_t sizeIo);

extern bool (*OSCreateThread)(OSThread *thread, void *entry, int argc,
                              char *argv, void *stack, uint32_t stackSize,
...
extern int (*sendto)(int s, const void *buffer, int size, int flags,
                     const sockaddr_in *dest, int dest_len);
extern int (*setsockopt)(int s, int level, int optname, void *optval,
                         int optlen);
extern int (*bind)(int sockfd, struct sockaddr *my_addr, int addrlen);
extern int (*listen)(int s, int backlog);
extern hostent *(*gethostbyname)(const char *ipAddr);
extern int (*accept)(int sock, struct sockaddr *addr, size_t *len);
...
extern const char *(*inet_ntoa)(in_addr addr);
...
extern bool (*OSSetExceptionCallbackEx)(OSExceptionMode mode,
...
extern int (*VPADRead)(int chan, VPADData *buffer, size_t buffer_size,
                       int *error);

extern int (*SAVEOpenFile)(FSClient *client, FSCmdBlock *block,
...
namespace nn::act {
extern uint8_t (*GetSlotNo)(void);
extern int (*GetAccountIdEx)(char *pOutAccountId, uint8_t slotNum);
extern int (*GetMiiName)(char16_t *pOutMiiName);
} // namespace nn::act

#define LOAD_FUNC(mod, x) ((__load_from_module(mod, #x, (void **)&x)))

namespace std {
void __attribute__((__noreturn__)) __throw_length_error(const char *);
}
