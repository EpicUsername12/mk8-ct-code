...
constexpr inline uint32_t CalculatePTEGAddress(uint32_t sdr1, uint32_t pageIndex, uint32_t hash) {
...
constexpr inline void GetValidPTEGFromAddr(PowerPC_MemoryRegisters* regs, uint32_t* pageTable, int srId, uint32_t addr, uint32_t* PTEH, uint32_t* PTEL) {
